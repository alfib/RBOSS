...
import com.mycompany.rboss.service.ShoppingCartService;
...
public class ShoppingCartController {
    ...
    public String addToCart(@PathVariable int id, Model model, Principal principal, int quantity,final RedirectAttributes re){
...
//       cartService.update(cart);
    ...
    public String editOrderLineQuantityplus(@PathVariable int id,Model model){
    ...
    public String editOrderLineQuantityminus(@PathVariable int id,Model model){
        OrderLineItem item = orderLineItemService.get(id);
        item.setQuantity(item.getQuantity()-1);
        orderLineItemService.update(item);
        return "redirect:/viewCart";
    ...
    public String editCartDeleteItem(@PathVariable int id,Model model, Principal principal){
        OrderLineItem item = orderLineItemService.get(id);
        User user = userService.getByUsername(principal.getName());
    ...
    public String checkout(Principal principal, Model model,final RedirectAttributes re){
        ...
        User user = userService.getByUsername(principal.getName());
//        
        if(user.getAddresses().isEmpty() || user.getCreditcards().isEmpty()){
